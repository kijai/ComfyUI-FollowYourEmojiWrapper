...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # clip fc
        self.clip_fc = nn.Linear(1024, 768, bias=True)
    ...
    def forward(
        ...
        lmk_cond_fea: Optional[torch.Tensor] = None,
        ...
        reference_mid_block_res_sample=None,
        reference_up_block_res_samples=None,
        ...
        encoder_hidden_states = self.clip_fc(encoder_hidden_states)
        ...
        if reference_down_block_res_samples is not None:
            is_reference = True
            reference_down_block_res_samples_list = []
            sub_list = []
            for i, reference_down_block_res_sample in enumerate(reference_down_block_res_samples):
                i = i + 1
                if i % 2 == 0:
                    sub_list.append(reference_down_block_res_sample)
                    reference_down_block_res_samples_list.append(sub_list)
                    sub_list = []
                else:
                    sub_list.append(reference_down_block_res_sample)

            reference_up_block_res_samples_list = []
            sub_list = []
            for i, reference_up_block_res_sample in enumerate(reference_up_block_res_samples):
                i = i + 1
                if i % 3 == 0:
                    sub_list.append(reference_up_block_res_sample)
                    reference_up_block_res_samples_list.append(sub_list)
                    sub_list = []
                else:
                    sub_list.append(reference_up_block_res_sample)
        else:
        ...
        nums = [i for i in range(len(self.down_blocks))]
        for i, downsample_block in zip(nums, self.down_blocks):
            if (
                ...
                sample, res_samples = downsample_block(
                    ...
                    reference_down_block_res_sample=reference_down_block_res_samples_list[i] if is_reference else None
                )
            else:
                sample, res_samples = downsample_block(
                    hidden_states=sample,
        ...
        sample = self.mid_block(
            ...
            encoder_hidden_states=encoder_hidden_states,
            attention_mask=attention_mask,
            reference_mid_block_res_sample=reference_mid_block_res_sample if is_reference else None
        )
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if (
                ...
                sample = upsample_block(
                    ...
                    attention_mask=attention_mask,
                    reference_up_block_res_sample=reference_up_block_res_samples_list[i - 1] if is_reference else None
                )
            else:
                sample = upsample_block(
                    hidden_states=sample,
    ...
    def from_pretrained_2d(
        ...
        if not isinstance(motion_module, str):
            motion_state_dict = motion_module.model.state_dict()
            if motion_module.model.mm_info.mm_format == "AnimateLCM":
                motion_state_dict = {k: v for k, v in motion_state_dict.items() if "pos_encoder" not in k}
            # merge the state dicts
            state_dict.update(motion_state_dict)
        else:
            motion_module_path = Path(motion_module)
    ...
    def set_do_classifier_free_guidance(self, do_classifier_free_guidance):
        ...
        self.mid_block.set_do_classifier_free_guidance(do_classifier_free_guidance)

        for module in self.up_blocks:
            module.set_do_classifier_free_guidance(do_classifier_free_guidance)

    def set_reentrant(self, use_reentrant):

        for module in self.down_blocks:
            module.set_reentrant(use_reentrant)

        self.mid_block.set_reentrant(use_reentrant)
