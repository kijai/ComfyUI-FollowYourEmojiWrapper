...
from .referencenet_blocks import (
    UNetMidBlock2D,
...
class ReferenceNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    ...
    def __init__(
        ...
        for i, up_block_type in enumerate(up_block_types):
            ...
            up_block = get_up_block(
                ...
                dropout=dropout,
                is_reference_last_layer=True if i == 3 else False,
                info_mode=info_mode
            )
    ...
    def forward(
        ...
        # sample.shape = torch.Size([2, 4, 98, 64])
        # np.max(sample.cpu().numpy()) = 8.914
        # np.min(sample.cpu().numpy()) = -10.3
        # encoder_hidden_states.shape = torch.Size([2, 1, 768])
        ...
        reference_down_block_res_samples = ()
        for downsample_block in self.down_blocks:
            if (
                ...
                sample, res_samples, reference_res_samples = downsample_block(
                ...
                reference_down_block_res_samples += reference_res_samples
            else:
        ...
        if self.mid_block is not None:
            if (
                ...
                sample, reference_mid_block_res_sample = self.mid_block(
                    sample,
                    emb,
                    encoder_hidden_states=encoder_hidden_states,
                    attention_mask=attention_mask,
                ...
                reference_mid_block_res_sample = reference_mid_block_res_sample[0]
            else:
        ...
        # 5. up
        reference_up_block_res_samples = ()
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if (
                ...
                sample, reference_res_samples = upsample_block(
                    hidden_states=sample,
                    temb=emb,
                ...
                reference_up_block_res_samples += reference_res_samples
            else:
        ...
        return reference_down_block_res_samples, reference_mid_block_res_sample, reference_up_block_res_samples
    ...
    def from_pretrained_2d(
        ...
        subfolder=None,
        referencenet_additional_kwargs=None,
    ):
        ...
        model = cls.from_config(unet_config, **referencenet_additional_kwargs)
